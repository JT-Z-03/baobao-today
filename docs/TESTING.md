...
## 2026-09-20 · 当前 1.2.0 / code 8

当前基线为 105 组、621 项 Jest 测试，业务 SQLite schema 9、独立工作状态库 schema 1；类型、lint、6 张品牌素材、12 张 UI 素材、Expo Doctor 21/21、public config 和 Android export 均通过。公开候选分支在原工作目录重新安装依赖并验证，不另建工程或构建镜像；源码文件清单和运行输入另行比对，内部文档与忽略文件不作为公开源码依赖。

本轮回归覆盖奶量空值与历史值、快捷补记跨日边界、五类草稿与保存防重、旧草稿隔离、亲喂计时与重启恢复、首页分钟/跨日刷新、外部备份写入和校验、恢复后的状态隔离，以及原生版本优先显示。API 36 模拟器验证代表性的填写保存、强制停止后恢复、照片草稿、计时切换与暂停、系统目录保存、ZIP 恢复及撤销流程；不是所有设备或故障组合的穷举。

使用者反馈 code 7 的全部功能测试无问题。code 8 的业务功能保持，另行完成 Android 14 同签名覆盖安装、实际 APK 哈希与设置页 1.2.0 (8) 核对；原首次安装时间及数据目录保持，但不据此声称整库、照片逐字节比较或新一轮完整功能验收。发布复用这个已安装的签名 APK。

未新增验证所有厂商的后台通知、真实存储耗尽或大体积照片备份；iOS、Web 尚未验收。公开安装包入口为 [v1.2.0](https://github.com/JT-Z-03/baobao-today/releases/tag/v1.2.0)，旧版标签保留。

## 历史自动化基线

2026-09-12 的 Android 1.1.1 / code 6 源码基线包含 97 个 Jest test suites、574 个测试，以及独立的 SQLite schema 检查。当前数据合同为 SQLite `user_version=9`；完整备份新建时写入 `formatVersion=2`，可恢复历史 `formatVersion=1` 和当前 `formatVersion=2`；CSV 固定 33 列，前 32 列保持兼容，第 33 列为 `breast_milk_amount_ml`；喝奶方式为奶粉、亲喂母乳、瓶喂母乳和混合。公开同步需核对完整文件清单、依赖和运行输入，并在公开候选分支重新安装依赖和执行必要检查。
...
当时安装包通过 `v1.1.1` Release 提供；`v1.1.0` 及 `ui-soft-rose-2026-09-09` 标签保留历史版本。2026-08-07 的19/20仅为历史检查记录。
