...
生成备份后，点击“保存到手机”并在系统界面选择目录。应用会创建独立文件、写入内容并读回校验；只有校验通过才显示已保存。下次可核对文件是否仍可访问。取消、写入失败或无法核对不会显示保存成功；分享给其他应用后，应到接收应用确认结果。

未完成草稿、草稿照片、尚未保存的亲喂计时和备份保存凭据不属于正式业务资料，不进入 CSV 或 ZIP。需要备份的记录应先保存。恢复会替换当前正式资料并清理本机未完成草稿，执行前页面会提示；撤销恢复也不会恢复旧草稿。
